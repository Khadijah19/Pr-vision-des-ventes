...
with st.sidebar:
    ...
    # ✅ plus de 12 ici
    weeks_window = st.selectbox(
        "⏱️ Fenêtre (semaines)",
        [10, 8, 4, 3, 2, 1],
        index=0,
        key="weeks_window"
    )
    st.caption(f"💡 Fenêtre max = {MAX_WEEKS} semaines (car parquet = {PARQUET_NAME})")
...
@st.cache_data(show_spinner=True)
def load_all(weeks_window):
    train = load_train_from_hf(weeks=int(weeks_window), filename="train_last10w.parquet")
    items = load_items_hf("items.csv")
    stores = load_stores_hf("stores.csv")
    return train, items, stores

train, items, stores = load_all(weeks_window)

    # ✅ On charge TOUJOURS depuis last10w, puis on sous-filtre weeks_window (<=10)
    train = load_train_recent(
        data_dir,
